...
import club.maxstats.kolour.util.wrap
    ...
    init {
        ...
        while (!executor.isTerminated) {
            ...
            while (!textureQueue.isEmpty()) {
                ...
                glTexImage2D(3553, 0, 6408, textureData.width, textureData.height, 0, 6408, 5121, textureData.buffer)
            }
        }
    }
...
class FontRenderer(
    ...
    private fun drawChar(
        ...
        glVertex2f(x, y);
        glTexCoord2f(0f, 1f);
        glVertex2f(x, y + charData.height);
        glTexCoord2f(1f, 1f);
        glVertex2f(x + charData.width, y + charData.height);
        glTexCoord2f(1f, 0f);
        glVertex2f(x + charData.width, y);
        glEnd();
    }
...
class GLFont(
    ...
    init {
        val textureIds = IntArray(256) { glGenTextures() }
